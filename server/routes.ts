...
Manual balance correction required IMMEDIATELY!`;
  ...
  app.get("/api/admin/blockchain-settings", requireAdmin, async (req, res) => {
    try {
      console.log("🔍 [BLOCKCHAIN-SETTINGS] Fetching all blockchain configurations...");
      
      const blockchainConfigs = await db.select()
        .from(schema.blockchainSettings)
        .orderBy(schema.blockchainSettings.chainName);
      
      console.log("✅ [BLOCKCHAIN-SETTINGS] Found configurations:", blockchainConfigs.length);
      res.json(blockchainConfigs);
    } catch (error) {
      console.error("❌ [BLOCKCHAIN-SETTINGS] Error fetching configurations:", error);
      res.status(500).json({ message: "Failed to fetch blockchain settings" });
    }
  });

  // Update blockchain setting
  app.patch("/api/admin/blockchain-settings/:id", requireAdmin, async (req, res) => {
    try {
      const settingId = parseInt(req.params.id);
      const updateData = req.body;
      
      console.log("🔧 [BLOCKCHAIN-SETTINGS] Updating blockchain setting:", settingId, updateData);
      ...
      // Update with current timestamp and admin user ID
      const updatedSetting = await db.update(schema.blockchainSettings)
        .set({
          ...updateData,
          updatedAt: new Date(),
          updatedBy: req.session.userId
        })
        .where(eq(schema.blockchainSettings.id, settingId))
        .returning();
      
      if (updatedSetting.length === 0) {
        return res.status(404).json({ message: "Blockchain setting not found" });
      }
      
      console.log("✅ [BLOCKCHAIN-SETTINGS] Updated successfully:", updatedSetting[0]);
      res.json(updatedSetting[0]);
    } catch (error) {
      console.error("❌ [BLOCKCHAIN-SETTINGS] Error updating setting:", error);
      res.status(500).json({ message: "Failed to update blockchain setting" });
    }
  });

  // Create new blockchain setting
  app.post("/api/admin/blockchain-settings", requireAdmin, async (req, res) => {
    try {
      const blockchainData = req.body;
      
      console.log("🔧 [BLOCKCHAIN-SETTINGS] Creating new blockchain configuration:", blockchainData);
      
      // Add admin user ID and timestamps
      const newBlockchainSetting = await db.insert(schema.blockchainSettings)
        .values({
          ...blockchainData,
          createdAt: new Date(),
          updatedAt: new Date(),
      ...
      console.log("✅ [BLOCKCHAIN-SETTINGS] Created successfully:", newBlockchainSetting[0]);
      res.json(newBlockchainSetting[0]);
    } catch (error) {
      console.error("❌ [BLOCKCHAIN-SETTINGS] Error creating setting:", error);
      res.status(500).json({ message: "Failed to create blockchain setting" });
    }
  });

  // Delete blockchain setting
  app.delete("/api/admin/blockchain-settings/:id", requireAdmin, async (req, res) => {
    try {
      const settingId = parseInt(req.params.id);
      
      console.log("🗑️ [BLOCKCHAIN-SETTINGS] Deleting blockchain setting:", settingId);
      
      if (isNaN(settingId)) {
        return res.status(400).json({ message: "Invalid setting ID" });
      }
      
      const deletedSetting = await db.delete(schema.blockchainSettings)
        .where(eq(schema.blockchainSettings.id, settingId))
        .returning();
      ...
      console.log("✅ [BLOCKCHAIN-SETTINGS] Deleted successfully:", deletedSetting[0]);
      res.json({ message: "Blockchain setting deleted successfully" });
    } catch (error) {
      console.error("❌ [BLOCKCHAIN-SETTINGS] Error deleting setting:", error);
      res.status(500).json({ message: "Failed to delete blockchain setting" });
    }
  });

  // Initialize default blockchain settings (one-time setup)
  app.post("/api/admin/blockchain-settings/init-defaults", requireAdmin, async (req, res) => {
    try {
      console.log("🔧 [BLOCKCHAIN-SETTINGS] Initializing default blockchain configurations...");
      
      // Check if settings already exist
      const existingSettings = await db.select().from(schema.blockchainSettings);
      if (existingSettings.length > 0) {
        return res.status(400).json({ message: "Blockchain settings already initialized" });
      }
      ...
      const defaultBlockchains = [
        {
          ...
          gasLimit: "21000",
          minConfirmations: 12,
          depositFeePercent: "0.00",
          withdrawalFeePercent: "2.50",
          tokenContracts: {
            USDC: "0xA0b86a33E6441E5bb5Bf4c0B6AC8E7a5CA58eb4F",
      ...
      const createdSettings = await db.insert(schema.blockchainSettings)
        .values(defaultBlockchains.map(blockchain => ({
          ...blockchain,
          createdAt: new Date(),
          updatedAt: new Date(),
          updatedBy: req.session.userId
      ...
      console.log("✅ [BLOCKCHAIN-SETTINGS] Initialized", createdSettings.length, "default blockchain configurations");
      res.json({ 
        message: "Default blockchain settings initialized successfully",
        settings: createdSettings
      });
    } catch (error) {
      console.error("❌ [BLOCKCHAIN-SETTINGS] Error initializing defaults:", error);
      res.status(500).json({ message: "Failed to initialize default blockchain settings" });
    }
  });
