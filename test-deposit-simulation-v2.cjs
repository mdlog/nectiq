...
async function testDepositSimulation() {
    console.log('🔍 Testing Deposit Simulation (v2)...');
    ...
    try {
        ...
        // Contract ABI for deposit function (fixed)
        const contractABI = [
        ...
        try {
            ...
            if (gasError.message.includes('ERC20: insufficient allowance')) {
                ...
                console.log('💡 ISSUE IDENTIFIED: Token not supported!');
                console.log('   USDC might not be configured in contract.');
            } else {
