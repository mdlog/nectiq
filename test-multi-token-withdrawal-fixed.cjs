...
/**
 * Test Multi Token Vault Withdrawal (Fixed)
 * 
...
// Test addresses (using valid checksum addresses)
...
async function testMultiTokenWithdrawal() {
    try {
        console.log('🧪 Testing Multi Token Vault Withdrawal (Fixed)');
        console.log('===============================================');
        ...
        // Step 6: Check user balance (using proper address formatting)
        console.log('\n👤 Checking user balance...');
        try {
            // Use proper address formatting
            const userAddress = ethers.getAddress(TEST_USER_ADDRESS);
            const tokenAddress = ethers.getAddress(USDC_ADDRESS);

            const userBalance = await vaultContract.getUserBalance(userAddress, tokenAddress);
        ...
        // Step 9: Check MultiTokenVaultEventListener
        console.log('\n👂 Checking MultiTokenVaultEventListener...');
        try {
            ...
            console.log(`   ❌ Error checking MultiTokenVaultEventListener: ${error.message}`);
        }
        ...
        console.log('   1. Check user has sufficient balance in vault');
        console.log('   2. Verify user has deposited tokens to vault first');
        console.log('   3. Check frontend is calling correct contract functions');
