...
- **✅ HEADER UI AUTHENTICATION FIX**: Fixed critical header UI authentication issue where profile dropdown displayed "Loading..." or "Not connected" despite successful wallet connection and user authentication (August 24, 2025). Root cause was header component using direct wagmi state instead of useRainbowAuth hook state. Fixed conditional logic to require both wallet connection AND user data before showing connected UI state. WebSocket authentication and backend sessions working correctly throughout.
...
- **✅ DOCUMENTATION SYNCHRONIZATION**: Updated NECTIQ Features Guide to match actual code implementation (August 23, 2025). Fixed major discrepancies in fee structures including battle platform fee (3.5%), corrected multiplier systems, added platform fee transparency, and documented actual reward formulas. Guide now accurately reflects the implemented accuracy-based system for regular predictions, exponential multiplier system for TrendRide, and corrected battle reward calculations with proper fee structure.
